...
export const useFileStore = create<useFileStoreProps>((set) => ({
    ...
    setCompletedFile: (file) => set((state) => ({ completedFiles: [...state.completedFiles, file] })),
    setSelectedFiles: (files) => set({ selectedFiles: files }),
    removePendingFile: (fileId) => set((state) => ({ pendingFiles: state.pendingFiles.filter(file => file.id !== fileId) })),
