...
export default function BlogDialog({ blog }: { blog?: Blog }) {
	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className=' h-[100vh]  max-w-8xl overflow-y-scroll'>
				...
				<Form {...form}>
					...
					>
						<div className=' space-y-2'>
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											{!image && (
												<div className=' p-12 bg-primary-foreground rounded-xl hover:border-white border border-dashed '>
													<UploadButton
														{...field}
							...
							/>
							{image && (
								<div className=' relative max-w-[300px] aspect-[16/10]'>
									<Image
										src={image}
										alt=''
										fill
								...
								<>
									...
									<div className='flex gap-2 flex-wrap mt-2'>
										{field.value?.map((tag?: string) => (
											<span
												key={tag}
												className='flex items-center px-4 p-1 text-sm gap-2 rounded-sm bg-secondary'
											>
												{tag}
												<button
													type='button'
